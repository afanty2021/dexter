...
- 心跳功能：实时市场数据监控
- 向量记忆：持久化记忆系统，支持语义搜索和时间衰减
- 定时任务：支持 at/every/cron 调度的定时任务系统
- 斜杠命令：快速访问常用功能（/model, /rules, /memory, /heartbeat 等）
...
graph TD
    ...
    click R "./src/tools/finance/CLAUDE.md" "查看 Finance 子模块详情"
    click S "./src/tools/search/CLAUDE.md" "查看 Search 子模块详情"
    click T "./src/tools/browser/CLAUDE.md" "查看 Browser 子模块详情"
```
...
| **UI 框架** | pi-mono (@mariozechner/pi-tui) | 终端用户界面 |
| **AI 框架** | LangChain | LLM 抽象与工具绑定 |
| **数据库** | better-sqlite3 | SQLite 数据库（Gateway、记忆、定时任务） |
| **浏览器自动化** | Playwright | 网页抓取与交互 |
| **定时任务** | croner | Cron 调度器 |
...
| 模块路径 | 主要职责 | 关键文件 | 入口文件 |
|---------|---------|---------|---------|
| **agent** | Agent 循环核心、提示词、上下文管理（已重构） | `agent.ts`, `prompts.ts`, `scratchpad.ts`, `run-context.ts`, `tool-executor.ts`, `channels.ts` | `src/agent/index.ts` |
...
| **commands** | 斜杠命令系统（/model, /rules, /clear, /memory, /heartbeat, /history, /help） | `index.ts` | `src/commands/index.ts` |
| **cron** | 定时任务系统（at/every/cron 调度、活跃时间、履行模式） | `runner.ts`, `executor.ts`, `schedule.ts`, `store.ts` | `src/cron/index.ts` |
| **memory** | 向量记忆系统（嵌入、搜索、时间衰减、MMR） | `database.ts`, `embeddings.ts`, `search.ts`, `indexer.ts`, `mmr.ts`, `temporal-decay.ts` | `src/memory/index.ts` |
...
| `bun dev` | 监听模式（开发） |
| `bun run gateway` | 运行 Gateway 服务器 |
| `bun run gateway:login` | Gateway 登录配置 |
...
### 2026-04-10 00:00:00 - 版本 2026.4.8 更新
- **CLI 性能优化**: 修复 CLI 闪烁和 UI 渲染性能问题
- **根渲染修复**: 修复根重新渲染问题
- **抑制功能**: 完善心跳抑制功能
- **版本更新**: v2026.4.8
...
*本文档由 AI 上下文初始化系统自动生成，最后更新：2026-04-10 00:00:00*
